...
class CoinMarketsSyncer extends Syncer {
  ...
  async syncHistorical(coinIds) {
    if (!coinIds && await CoinMarket.exists()) {
      return
    }
  ...
  async adjustPoints(dateFrom, dateTo) {
    await CoinMarket.deleteExpired(dateFrom, dateTo)
  }
  ...
  async sync({ dateFrom, dateTo, period }, coinIds) {
    ...
    logger.info(`Start syncing historical markets for period:${period}, /${coins.length} coins`)
    ...
    for (let coinsIndex = 0; coinsIndex < coins.length; coinsIndex += 1) {
      try {
        ...
        if (coinMarkets.length > 0) {
          this.upsertCoinMarkets(coinMarkets)
        }
        ...
        if (response.status === 429) {
          logger.error(`Sleeping 1min (histo-markets sync); Status ${response.status}`)
          await utils.sleep(60000)
        }

        if (response.status >= 502 && response.status <= 504) {
          logger.error(`Sleeping 30s (histo-markets sync); Status ${response.status}`)
          await utils.sleep(30000)
        }
      }
    }

    logger.info(`Successfully synced historical markets for period: ${period}`)
  ...
  upsertCoinMarkets(coinMarkerts) {
    CoinMarket.bulkCreate(coinMarkerts, {
...
module.exports = CoinMarketsSyncer
